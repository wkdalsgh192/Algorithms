...
public class Solution1812_수정이의타일자르기 {
	...
	private static void cut() {
		...
		queue.offer(new Rectangle(M,M)); // 타일 1장을 사고 시작
		cnt++;
		
		for (int i = N-1; i >= 0; i--) {
			go(1 << size[i]); // 만들고자하는 타일의 크기를 줌
		}
	}
	...
	private static void go(int size) {
		...
		// 원하는 크기의 타일을 만들 수 있으면 만들고 잘라낸 2개의 자투리 타일을 다시 보관
		if (r.min >= size) {
